...
func (repo *SQLRepository) SaveOrder(order *models.Order) {
	...
	for _, sel := range order.Products {
		...
		if err != nil {
			repo.Logger.Panicf("Cannot exec SaveOrderLineCommand: %v", err.Error())
			tx.Rollback()
			return
		}
	}
	err = tx.Commit()
	if err != nil {
		repo.Logger.Panicf("Transaction cannot be commited: %v", err.Error())
		err = tx.Rollback()
		if err != nil {
			repo.Logger.Panicf("Transaction cannot be roled back: %v", err.Error())
		}
	}
	order.ID = int(id)
}
