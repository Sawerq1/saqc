		...
		if err == nil {
			for lineRows.Next() {
				...
				err = lineRows.Scan(&ps.Quantity, &ps.Product.ID, &ps.Product.Name,
					&ps.Product.Description, &ps.Product.Price, &ps.Product.Category.ID, &ps.Product.Category.CategoryName)
				if err == nil {
			...
			repo.Logger.Panicf("Cannot exec GetOrderLine command: %v", err.Error())
		}
	} else {
